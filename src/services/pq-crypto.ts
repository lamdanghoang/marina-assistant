...
const PQ_SECRET_KEY = 'marina_pq_secret_key';
const PQ_PUBLIC_KEY = 'marina_pq_public_key';
...
export async function hasLocalPQKey(): Promise<boolean> {
  const sk = await SecureStore.getItemAsync(PQ_SECRET_KEY);
  return !!sk;
}
...
export async function savePQKeypair(publicKey: Uint8Array, secretKey: Uint8Array): Promise<void> {
  ...
  await SecureStore.setItemAsync(PQ_SECRET_KEY, toBase64(secretKey));
  await SecureStore.setItemAsync(PQ_PUBLIC_KEY, toBase64(publicKey));
}
...
export async function getLocalPublicKey(): Promise<Uint8Array | null> {
  const { fromBase64 } = await import('@mysten/sui/utils');
  const pk = await SecureStore.getItemAsync(PQ_PUBLIC_KEY);
  if (!pk) return null;
